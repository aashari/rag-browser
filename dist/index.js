  ...
  try {
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        if (entry.entryType === "layout-shift" && !layoutShift.hadRecentInput && layoutShift.value > SHIFT_THRESHOLD) {
          significantShifts++;
...
async function injectStabilityScripts(page) {
  try {
    ...
    const isInjected = await page.evaluate(() => {
      return window.stabilityScriptsInjected === true && typeof window.getFullPath === "function" && typeof window.checkPageStability === "function" && typeof window.checkLayoutStability === "function";
    }).catch(() => false);
    if (isInjected) {
    ...
    const verified = await page.evaluate(() => {
      return typeof window.getFullPath === "function" && typeof window.checkPageStability === "function" && typeof window.checkLayoutStability === "function";
    }).catch(() => false);
    if (!verified) {
...
async function waitForActionStability(page, options = {}) {
...
async function getElementInfo(page, element) {
  const info2 = await element.evaluate((el) => {
    ...
    if (!label) {
      label = htmlElement.getAttribute("aria-label") || htmlElement.getAttribute("aria-labelledby")?.split(" ").map((id) => document.getElementById(id)?.textContent?.trim()).filter(Boolean).join(" ") || "";
    }
    if (!label) {
    ...
    if (!label) {
      label = htmlElement.getAttribute("placeholder") || htmlElement.getAttribute("data-placeholder") || htmlElement.getAttribute("aria-placeholder") || "";
    }
    ...
    if (!label) {
      label = htmlElement.getAttribute("name") || htmlElement.id || htmlElement.getAttribute("role") || "No label";
    }
    const isVisible = (element2) => {
      ...
      if (element2.parentElement) {
        return isVisible(element2.parentElement);
      }
      return true;
    };
    return {
...
if (fileURLToPath(import.meta.url) === process.argv[1]) {
  main().catch((error2) => {
    console.error("Fatal error:", error2 instanceof Error ? error2.message : String(error2));
    process.exit(1);
  });
}
