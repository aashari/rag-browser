...
export async function executeAction(page: Page, action: Action, options: BrowserOptions): Promise<ActionResult> {
  try {
    switch (action.type) {
      case 'wait': {
        await Promise.all(action.elements.map(selector => 
      ...
      case 'print': {
        ...
        return {
          ...
          warning: results.some(r => r.error) ? 
            `Failed to capture some elements: ${results.filter(r => r.error).map(r => r.selector).join(', ')}` : 
            undefined,
          data: results
        };
      }
      ...
      default:
        return {
          ...
          error: `Action type ${(action as any).type} not supported`
        };
    }
  } catch (error) {
...
export async function executePlan(page: Page, plan: Plan, options: BrowserOptions): Promise<{
  ...
  // Print final summary
  printActionSummary(actionStatuses);
